...
data {
  ...
  int<lower=0>             ncen;             // number of censored event times
  int<lower=0>             Ncen;             // number of longitudinal measurements for censored event times
  array[Ncen] int<lower=1> idcen;            // idcen variable for censored event times
  vector[Ncen]             ycen;             // longitudinal measurements for censored event times
  vector<lower=0>[Ncen]    visitcen;         // Visit times for censored individuals
  matrix[Ncen, plong]      Xlong_cen;        // Design matrix for longitudinal outcome (censored)
  vector<lower=0>[ncen]    tcen;             // Event times (censored)
  matrix[ncen, ptte]       Xtte_cen;         // Design matrix for TTE outcome (censored)
...
parameters {
  ...
  // vector<lower=tcen>[ncen] tcenimp;   // Imputed event times for censored individuals
  ...
  // real<lower=0.1,upper=10>        chgpt_sd;           // SD of change point
  ...
  // vector<lower=0,upper=1>[ncen]   chgpt_cen_raw;      // raw version of change point random effect U(0, 1)
  // matrix[3, nobs]                 b_obs_raw;          // raw version of random coefficients (will be std. normal)
  // matrix[3, ncen]                 b_cen_raw;          // raw version of random coefficients (will be std. normal)
}
// transformed parameters {
  // vector[nobs] chgpt_obs;           // changepoint (observed times)
  // vector[ncen] chgpt_cen;           // changepoint (censored times)
  // matrix[3,nobs] b_obs;             // random intercept and slopes (observed event times)
  // matrix[3,ncen] b_cen;             // random intercept and slopes (censored event times)
  ...
  // chgpt_cen = tcenimp .* chgpt_cen_raw;    // in [0, tcenimp] REQUIRES JACOBIAN ADJUSTMENT SINCE tecenimp IS RANDOM
  ...
  // b_cen   = randeff_cov_chol[2:4,2:4] * b_cen_raw;             // implies b_obs | chgpt_obs ~ MVN(0, condCov)
  // for ( i in 1:nobs )
  //   b_obs[, i] += b_mean + LHS * (chgpt_obs[i] - chgpt_mean);  // implies b_obs | chgpt_obs ~ MVN(condMean, condCov)
  // for ( i in 1:ncen )
  //   b_cen[, i] += b_mean + LHS * (chgpt_cen[i] - chgpt_mean);  // implies b_cen | chgpt_cen ~ MVN(condMean, condCov)
// }
model {
  // vector[Nobs] yobs_mean = Xlong_obs * beta_y;
  // vector[Ncen] ycen_mean = Xlong_cen * beta_y;
  // real diff_ij;
  ...
  // }
  // for ( i in 1:Ncen ) {
  //   diff_ij = visitcen[i] - chgpt_cen[idcen[i]];
  //   ycen_mean[i] += b_cen[1,idcen[i]] + (diff_ij <= 0 ? b_cen[2,idcen[i]] : b_cen[3,idcen[i]] ) * diff_ij;
  // }
  ...
  tcen              ~ weibull_ph_survival(Xtte_cen, beta_tte, shape_tte, scale_tte);
  // ycen                 ~ normal(ycen_mean, sd_y);                                // conditional distribution of ycen | random effects
  // chgpt_cen            ~ tnormal(chgpt_mean, randeff_cov_chol[1,1], tcenimp);
  // to_vector(b_cen_raw) ~ std_normal();
  // target += log(tcenimp); // jacobian adjustment for change point
